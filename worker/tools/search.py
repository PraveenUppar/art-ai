...
def collect_evidence_with_domains(topic: str, domains: list[str]) -> dict[str, str]:
    ...
    firecrawl_client = FirecrawlApp(api_key=settings.fire_crawl_api_key)

    for query in queries:
        search_results = _safe_search(firecrawl_client, query)
        formulated_results = []
...
def _process_single_question(
    domain: DomainType, question: FactCheckItem, firecrawl_app: FirecrawlApp
):
...
def collect_evidence(questions: list[FactCheckItem], domains: list[DomainType]):
    ...
    def process_and_append(domain, question):
        ...
        firecrawl_app = FirecrawlApp(api_key=settings.fire_crawl_api_key)
    ...
    with ThreadPoolExecutor(
        max_workers=min(10, len(domains) * len(questions))
    ) as executor:
